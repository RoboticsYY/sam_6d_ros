...
def ov_to_torch(x):
    ...
    if isinstance(x, np.ndarray):
        ...
        return x
    elif isinstance(x, ov.utils.data_helpers.wrappers.OVDict):
        out_list = []
        for idx in range(len(x)):
            item = x[idx]
            if isinstance(item, np.ndarray):
                out_list.append(torch.from_numpy(item))
            else:
...
class BasePredictor:
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        # NOTE: Please change the path below to your own model's .xml file
        # Example: "/path/to/your_model.xml"
        fastsam_yolo_v8_predictor_model_path = ""
        self.ov_segmentor = get_ov_model(fastsam_yolo_v8_predictor_model_path, device="GPU")
    ...
    def inference(self, im, *args, **kwargs):
        ...
        ### hack with ov model
        # out_torch = self.model(im, augment=self.args.augment, visualize=visualize)
        out = self.ov_segmentor(im.numpy())
        out = ov_to_torch(out)
        # for idx, item in enumerate(out_torch):
        #     if isinstance(item, torch.Tensor):
        #         print(idx, item.shape)
        #     else:
        #         print(idx, type(item), len(item))
        #         print(item[0][0].shape, item[0][1].shape, item[0][2].shape)
        #         print(item[1].shape, item[2].shape)
        # print("="*20)
        # for item in out:
        #     print(item.shape)
        return out[0], (out[1:4], out[4], out[5]) # self.model(im, augment=self.args.augment, visualize=visualize)
    ...
    def stream_inference(self, source=None, model=None, *args, **kwargs):
        ...
        for batch in self.dataset:
            ...
            # Inference
            with profilers[1]:
                # print("==> Inference FastSAM from: ", __file__)
